/**
 * Trigger on delete a Deal Action (Deal_Action__c).
 *
 * When a deal action is delete, four fields need to be updated on the related contact and deal.
 * This happens on after delete to be sure that Deal Action is currently delete and maintain coherence of the fields.
 */
trigger DealActionDelTrigger on Deal_Action__c (after delete) {
    ...
    for (Id dealActionId : Trigger.oldMap.keySet()) {
        // check if changes applied
        Deal_Action__c dealAction = Trigger.oldMap.get(dealActionId);
        DealActionTriggerEnum dealActionTriggerEnum = DealActionTriggerEnumHandler.getDealActionTriggerEnum(null, dealAction.Action__c, true);
